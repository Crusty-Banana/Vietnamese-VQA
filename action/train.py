...
def train(model, train_loader, val_loader, optimizer, criterion, num_epochs, scheduler, device, run_dir, run_name):
    ...
    logging.info(f"Total number of parameters in the model: {sum(p.numel() for p in model.parameters())}")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_dict in train_bar:
            ...
            img_ws = batch_dict['img_w'].to(device)
            ...
            # Forward pass
            outputs = model(input_ids, img_ws, attention_mask, labels=labels)
            loss = outputs['loss']
