...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        img_w: Optional[torch.FloatTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        decoder_input_ids: Optional[torch.LongTensor] = None,
        decoder_attention_mask: Optional[torch.BoolTensor] = None,
        head_mask: Optional[torch.FloatTensor] = None,
        ...
        cross_attn_head_mask: Optional[torch.Tensor] = None,
        encoder_outputs: Optional[Tuple[Tuple[torch.Tensor]]] = None,
        image_id: Optional[Tuple[Tuple[torch.Tensor]]] = None,#**
        ...
        # normalize after
        normalize = nn.LayerNorm(hidden_states.size()[-1]).to('cuda')
        hidden_states = normalize(hidden_states).to('cuda') # layer Normalization 
        
        img_attention_mask = torch.ones(img_w.size()[0],img_w.size()[1]).to('cuda') # (batch_size x 196)
        
        attention_mask = torch.cat((img_attention_mask,attention_mask), -1)
